...
import { Folder, Star, Trash, X } from "lucide-react";
import {
  Table,
  TableHeader,
  TableColumn,
...
export default function FileList({
  ...
  // Enhanced Smart Search - searches name, tags, and AI description
  const filteredFiles = useMemo(() => {
    let filtered = [];
    
    switch (activeTab) {
      case "starred":
        filtered = files.filter((file) => file.isStarred && !file.isTrash);
        break;
      case "trash":
        filtered = files.filter((file) => file.isTrash);
        break;
      case "all":
      default:
        filtered = files.filter((file) => !file.isTrash);
    ...
    // Apply SMART search filter (searches name, tags, AI description)
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter((file) => {
        ...
        const descriptionMatch = file.aiDescription?.toLowerCase().includes(query) || false;
        
        // Search in suggested folder
        const folderMatch = file.aiSuggestedFolder?.toLowerCase().includes(query) || false;
        
        return nameMatch || tagsMatch || descriptionMatch || folderMatch;
      });
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {filteredFiles.length === 0 ? (
        ...
        <Card shadow="sm" className="border border-default-200 bg-default-50 overflow-hidden">
                    ...
                    <TableCell>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {/* Show AI-generated tags with search highlighting */}
                          <FileTags file={file} searchQuery={searchQuery} />
